...
const AdminDashboard = () => {
  ...
  const chartData = useMemo(() => {
    ...
    const distribution = projects.reduce((acc, proj) => {
      const supervisorName = proj.supervisor?.name || "Unassigned";
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 pb-24 pt-12 px-8">
        <div className="max-w-7xl mx-auto text-white">
          ...
          <p className="opacity-80">Managing {projects.length} active projects and {Alluser.length} system users.</p>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-8 -mt-16 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <TopStat title="Total Students" value={stats?.totalStudents} Icon={GraduationCap} border="border-l-blue-400" bgColor="bg-blue-50" />
          <TopStat title="Total Teachers" value={stats?.totalTeachers} Icon={Users} border="border-l-green-400" bgColor="bg-green-50" />
          <TopStat title="Pending Requests" value={stats?.pendingRequests} Icon={AlertCircle} border="border-l-orange-400" bgColor="bg-orange-50" />
          <TopStat title="Active Projects" value={projects.length} Icon={FolderKanban} border="border-l-yellow-400" bgColor="bg-yellow-50" />
          <TopStat title="Completed" value={stats?.completeProjects} Icon={AlertCircle} border="border-l-red-400" bgColor="bg-red-50" />
        </div>
                ...
                <ResponsiveContainer>
                  <BarChart data={chartData}>
                    ...
                    <YAxis 
                      ...
                      tickLine={false} 
                      fontSize={12} 
                      allowDecimals={false} // Corrected: Put on YAxis
                      domain={[0, 'auto']} 
                    />
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="flex flex-wrap gap-4">
            <button 
              ...
              className="flex-1 bg-blue-600 text-white py-3 rounded-lg flex items-center justify-center gap-2 font-medium hover:bg-blue-700 transition-all"
            >
              <Plus size={18} /> Add Student
            </button>
            <button 
              onClick={() => navigate("/admin/teachers")} 
              className="flex-1 bg-green-600 text-white py-3 rounded-lg flex items-center justify-center gap-2 font-medium hover:bg-green-700 transition-all"
            >
              <Plus size={18} /> Add Teacher
            </button>
            <button 
              onClick={() => navigate("/admin/projects")} 
              className="flex-1 border border-blue-600 text-blue-600 py-3 rounded-lg flex items-center justify-center gap-2 font-medium hover:bg-blue-50 transition-all"
            >
